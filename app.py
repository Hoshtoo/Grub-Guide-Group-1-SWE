...
# load_dotenv() only affects variables not already set in the environment.
# Codespace/Replit/Render secrets always take priority over .env,
# and local .env is used as a fallback for development.
load_dotenv()
...
def health():
    ...
    # Detect which environment the credentials came from
    in_codespace = bool(os.environ.get("CODESPACES"))
    source = (
        "GitHub Codespaces" if in_codespace
        else ".env file or platform secrets"
    )
 
    return jsonify({
        "status":                 "ok",
        "credentials_configured": has_creds,
        "credential_source":      source if has_creds else "not found",
    })
...
if __name__ == "__main__":
    ...
    print("\n" + "═" * 50)
    print("  Kroger Price Calculator — Flask Backend")
    print("═" * 50)
    if client_id and client_secret:
        source = "GitHub Codespaces" if in_codespace else ".env / platform secrets"
        print(f"  ✓ Credentials loaded from: {source}")
        print(f"  ✓ Client ID: {client_id[:8]}{'*' * (len(client_id) - 8)}")
    else:
        print("  ✗ WARNING: Credentials not found!")
        print("    Set KROGER_CLIENT_ID and KROGER_CLIENT_SECRET")
        if in_codespace:
            print("    In Codespaces: github.com > Settings > Codespaces > Secrets")
            print("    Then stop and restart your Codespace.")
        else:
            print("    Locally: create a .env file in the project root.")
