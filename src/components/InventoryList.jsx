...
function groupByCategory(items) {
    ...
    Object.keys(groups).forEach((cat) => {
        if (!CATEGORY_ORDER.includes(cat)) sortedEntries.push([cat, groups[cat]])
    })

    return sortedEntries
}

function InventoryList({ items, onDelete, onEdit, onAddToList, duplicateItemIds = new Set() }) {
    if (items.length === 0) {
        return (
            <div style={styles.empty}>
                <p style={styles.emptyText}>No items found</p>
                <p style={styles.emptySubtext}>Add items above or adjust search and filters</p>
            </div>
        )
    }
